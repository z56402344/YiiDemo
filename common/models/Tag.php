...
class Tag extends \yii\db\ActiveRecord
{
    ...
    public static function string2arr($tags){
        return preg_split('/\s*,\s*/',trim($tags),-1,PREG_SPLIT_NO_EMPTY);
    }

    public static function arr2string($tags){
        return implode(', ',$tags);
    }
    ...
    public static function updateFrequency(){
        if (!empty($oldTags) || !empty($newTags)){
            $oldTagsArray = self::string2arr($oldTags);
            $newTagsArray = self::string2arr($newTags);
